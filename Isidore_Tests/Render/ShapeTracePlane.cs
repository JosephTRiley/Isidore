    ...
    class ShapeTracePlane
    {
        ...
        public static bool Run()
        {
            ...
            Isidore.Maths.Point centPt = new
                Isidore.Maths.Point(0, 0, 0);
            Normal surfNorm = new Normal(0, 0, -1);
            Vector up = new Vector(0, 1, 0);
            double ulen = 0.5;
            double vlen = 0.5;
            Isidore.Render.Plane plane = new Isidore.Render.Plane(centPt,
                surfNorm, up, ulen, vlen);
            ...
            plane.TransformTimeLine.AddKeys(trans1, 5.0);
            plane.TransformTimeLine.AddKeys(trans2, 10.0);

            // Orthonormal projector 
            // Located -10m from the shape
            RectangleProjector proj = new RectangleProjector(120, 140, 
                0.01, 0.01, 0, 0);
            proj.TransformTimeLine = new KeyFrameTrans(Transform.Translate(
                new double[]{0,0,-10}));
            ...
            for (int idx = 0; idx < times.Length; idx++)
            {
                ...
                if (idx == bfOffIdx)
                    plane.IntersectBackFaces = false;
                else
                    plane.IntersectBackFaces = true;
                ...
                watch.Stop();
                Console.WriteLine("Plane Shape Trace Render, frame {0}: {1}ms", idx + 1, watch.ElapsedMilliseconds);

                // Displays frame, & records position (World space)
                ...
                for (int idx0 = 0; idx0 < len0; idx0++)
                {
                    for (int idx1 = 0; idx1 < len1; idx1++)
                    {
                        ...
                        if (thisRay.IntersectData.Hit)
                        {
                            ...
                            int index = 0; // Indexer
                            for (int ridx = 0; ridx < thisRay.IntersectData.Properties.Count; ridx++)
                            {
                                // Extracts type
                                string pType = thisRay.IntersectData.Properties[ridx].GetType().ToString();

                                if (pType == "Isidore.Render.Reflectance")
                                {
                                    Reflectance thisReflect = (Reflectance)thisRay.IntersectData.Properties[ridx];
            ...
            matlab.Execute("ShapeTracePlaneCheck;");
